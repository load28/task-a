...
Domain과 Engine은 OpenCode에 의존하지 않고, MCP·HTTP는 동일한 Task Agent 계약에 연결된다. 다만 앞선 “설계 전체 구현 완료” 보고는 실제 검증 범위를 초과했다. 현재는 SQLite 단일 서비스 구현이며, 모델을 포함한 자연어 전체 흐름은 검증하지 않았다.
...
- sync는 OpenCode가 이벤트를 추출하고 Core가 원자적으로 저장한다. 원래 설계의 “OpenCode가 Tool Loop에서 직접 sync 이벤트를 기록”하는 흐름은 아직 구현하지 않았다. run에서만 실제 Task Tool 호출이 가능하며 실제 모델 호출 검증이 남아 있다.
- Context Compiler는 목적별 결정적 필드 선택이다. 의미 기반 관련성 선정, 명시적 토큰 예산, Next Action 완료·Constraint 철회 lifecycle은 아직 없다.
