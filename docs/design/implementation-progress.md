...
상태: 구현 완료. 원문 4,997행·158개 요구 단위·191개 의미 계약을 보존하며, T01–T15의 연결된 실행 경로를 검증했다. 실제 provider 성공 호출은 사용자가 유지하기로 한 외부 검증 예외다.
...
현재 region 선택은 read/process completeness가 입증되지 않으면 보수적인 포함 영역을 요구한다. `replanner.selection`이 등록되면 finite connected union 후보를 생성하고 실제 등록 검증기가 feasibility와 planning/reasoning/context/reexecution/integration/interruption/discardedWork/warmSessionLoss/dataMigration/expectedFailure 비용을 공통 단위로 평가한다. 평가 이전에는 replanner를 호출하지 않는다. 입력 변경, 실패·비구조 평가, 비용 단위 불일치, unknown feasibility는 안전한 최소성 주장으로 바꾸지 않는다. 하한 탐색은 평가 한도·후보 누락·unknown을 gap에 남긴다. `minimumProven`은 등록된 finite domain과 보수적 closure 제약 안의 평가 비용에만 해당하며, 프로그램 전체의 최소 영역이나 실제 미래 비용의 최적성을 의미하지 않는다. 선택 정책이 없으면 기존 보수적 경로를 유지하고 minimumProven=false로 기록한다. 현재의 완전한 boundary proof가 있으면 가장 작은 검증 경계로 전파를 제한한다. 불확실성을 포함한 keep/switch는 현재 계획 유효성과 비용 구간을 사용한다.

동시에 들어온 변경은 각각의 실제 affected closure를 계산한 뒤 closure·보존 boundary·공유 contract/resource·causal link·계층적 write scope·물리 reservation의 overlap graph로 분할한다. 전이적으로 겹친 변경은 한 component로 합치고, 완전한 보존 경계로 서로 독립임이 확인된 component에는 같은 union lease 아래 별도 replanner grant를 발급한다. 각 결과는 자기 component 밖의 기존 node를 건드릴 수 없다. 질문은 해당 component의 grant만 새로 발급하며 다른 component의 완료 결과를 보존한다. 모든 component patch는 controller가 결합하고 전체 합성 계획을 등록 validator로 다시 검증한 뒤 하나의 revision으로 원자 커밋한다. 따라서 병렬 계획이 전역 generation 경쟁이나 두 번째 stale commit을 만들지 않는다.

`RegionCostCalibration`은 완료된 지역 복구의 모든 replanner 호출과 교체 task 실행에서 controller가 채택한 실제 input/output token, tool call, elapsed receipt를 등록된 공통 단위로 환산한다. integration, interruption, discarded work, warm session loss, data migration, expected failure는 불변 성분별 영수증으로만 실제값에 더한다. 완료 뒤 늦게 도착한 성분 영수증도 별도 내구 event로 다시 평가하며, 추정값이 0이 아닌 성분의 영수증이 하나라도 없으면 표본을 만들지 않는다. 성공한 완료 conjunction은 실제 expected failure 0의 근거가 되며 integration validator의 실행 시간은 자동 수집한다. 나머지 운영 성분은 해당 adapter가 실측 영수증을 제출해야 한다. calibration key는 validator·비용 단위·환산 정책 버전에 고정한다. 최소 표본 수와 최대 상대 오차를 모두 통과한 경우에만 관측된 최대 비율을 보수 계수로 이후 후보 비용 전체에 적용한다.
...
`PolicyReplay`는 실제 specialist router의 activate/skip/defer 판단 시 신호·역할/정책 버전·호출 횟수·cooldown·eligibility·task snapshot·당시 인과 그래프·근거를 불변 frame으로 보존한다. 후보 등록 시 outbox cutoff와 episode 단위 training/holdout 분할을 고정한다. 과거 frame은 historical, 이후 frame은 shadow로 재현하며 재시작·재전달에 동일한 판단과 분할을 유지한다. activation의 additional-trigger 후보는 기존 hard trigger·quota·cooldown·eligibility를 약화하지 않는다. 관측하지 않은 relation/scope는 false가 아니라 unknown이다.

`PolicyApplications`는 요청 생성 시 activation/context/decomposition/role/validation/integration/escalation/cache/precision/propagation/boundary/expectation/routine의 13개 head와 revision을 하나의 불변 bundle로 고정한다. 이후 head가 이동해도 진행 중 요청과 grant는 이 bundle을 유지한다. 구조 규칙은 현재 feature·relation·change scope에 대해 true/false/unknown으로 평가하며 true일 때만 target과 같은 discriminated effect를 합성한다. 자유 형식 invariant 문자열은 실행 설정이 될 수 없다. 각 평가와 effective program은 수정 불가능한 원장 및 `PolicyApplied` event에 남는다.

각 효과는 기존 실행 경계에 직접 연결된다. activation은 등록 specialist 요구, context는 role selector와 네 budget, decomposition은 계획 task 상한, role은 planner/worker/replanner의 등록 role/profile, validation은 단계별 validator obligation, integration은 일곱 validator, escalation은 질문·입력 재계획·국소 복구 quota를 바꾼다. cache는 L0 재사용 허용, precision은 worker 최저 profile, propagation은 인과 임계값, boundary는 실제 verified crossing edge와 binding validator를 요구하는 완전 경계, expectation은 실행 전 prediction policy, routine은 허용된 불변 routine 집합을 제어한다. 등록되지 않은 role·validator·routine 참조는 실행 전에 거절한다. 합성된 프로그램 hash를 grant와 L1 preflight에 고정해 기본 프로그램과 정책 적용 결과를 혼동하지 않는다.
...
정책 head의 변경 횟수는 DB trigger로 보존한다. 검증 대기 중 다른 정책으로 이동했거나 원래 버전으로 돌아온 경우에도 오래된 rollback을 거절한다. 진행 중 grant의 bundle·context·상태는 변경하지 않는다. sample 한도 소진과 superseded 상태는 명시적인 종료 event로 기록한다. 명령 실패·실행 오류·비구조 응답·출력 잘림은 unknown이며 회귀 증명이 아니다. 실행 오류 event에는 실패한 job과 obligation을 남겨 대기 sample을 종료한다.
...
native 검증은 macOS seatbelt 또는 Linux bubblewrap 안에서 실행한다. Linux는 사용자·mount·PID·네트워크 namespace를 분리하고 host root를 mount하지 않는다. 작업 공간과 명시된 런타임은 읽기 전용이며, 쓰기는 작업별 임시 공간으로 제한한다. cwd는 작업 공간 안으로 한정한다. macOS에서는 환경 상속·네트워크·IPC·process fork를 금지하며, 다중 프로세스 검증은 격리 Pod에서 수행한다. Windows와 bubblewrap이 없는 Linux는 격리 없이 실행하지 않고 Pod를 요구한다. 시간 초과 시 실제 프로세스를 종료하고 임시 공간을 정리한다. 이 접근 통제는 모든 파일/환경 읽기의 dependency completeness 증명을 대신하지 않는다.

## 검증 근거

- `npm run check`: TypeScript 검사와 전체 432개 테스트 통과, 실패·skip 0. 초기 요청→실제 plan validator→허가된 worker→실제 파일 관찰→완료, QA 조건부 활성화, 국소 복구, 독립 region 병렬 계획과 합성 검증, 검증된 재계획→대체 실행, 방향 수정의 목표 보존을 포함한다. 모델 부분은 합성 executor를 사용한다.
...
재계획 중 더 새로운 측정 원인이 도착하면 기존 repair를 superseded로 보존하고 그 generation의 grant·lease·stage를 fence한다. 새 허가에는 최신 유효 원인과 근거를 전달한다. 이전 실행의 응답 대기 중에도 dispatcher가 실제 종료 확인을 수행한다. 종료가 불명확하면 stopping 및 미확정 예산을 유지한다. 새 시도도 설정된 maxAttempts를 소비하며 횟수나 예산을 초기화하지 않는다. 동일 batch 안의 독립 region은 위의 병렬 component 경로를 사용하고, 이미 시작된 episode 뒤에 도착한 변경은 이 순차 supersession 경로를 사용한다.
...
`AdversarialReview`는 worker·planner·replanner L5를 단일 호출의 이름으로 취급하지 않는다. program에 등록된 서로 다른 읽기 전용 reviewer grant를 각각 발급하고, 각 결과의 실제 독립 검증 이후 등록 joint validator가 모든 결론을 함께 판정한다. task 결과는 현재 관찰 tuple에, 인지 결과는 grant의 현재 입력 vector에 고정한다. 두 검토의 개별 성공만으로 worker 완료·최초 계획 채택·지역 patch 검증을 진행하지 않는다. joint 실패는 worker에서는 실제 실패 근거로 scoped 재계획에 연결하고, 인지 결과에서는 해당 제안을 거절한다. specialist reviewer 자체의 L5 재귀는 유한하지 않으므로 등록 단계에서 거절한다.
...
최신 검증: 타입 검사와 전체 432개 테스트 통과(`npm run check`). 원문 추적 검사에서 4,997행의 누락·중복과 미매핑 필드는 0이다. 같은 최신 소스를 별도 로컬 이미지로 빌드하고 호스트·이미지의 source hash 일치를 확인했다. 해당 이미지로 실제 kind에서 읽기 전용 PVC·자격 증명 제외·커널 네트워크 차단·semantic receipt·controller의 verified 채택을 다시 검증했고 임시 namespace 삭제를 확인했다. 이미지 ID와 source hash는 `kubernetes-grant-validation.json`에 기록했다. 실제 provider 호출은 추가하지 않았다.
...
중단 대기 중 연속 변경은 다음 episode의 현재 원인으로 병합한다. 원래 요청·기대치·이미 사용한 모델 예산·repair 횟수는 유지하며 quota 소진 시 미해결 상태를 유지한다. DB 재시작, 폐기 트랜잭션의 강제 실패와 rollback, 중복 폐기, 종료 확인 전 발급 차단, 폐기된 revision의 재승인 차단, 새 revision 실행과 최종 완료를 검증했다. 독립 region 병렬 처리도 같은 union lease와 합성 검증 경로에 연결했다.

최신 전체 검사 422개에는 승인 전이, causal edge completeness 승격, 경계 보존, keep/switch, 독립 region 병렬 계획과 합성 검증, 실제 모델 실행 비용 보정, dispatcher 소유권 lease, granted write lineage, 역할 생명주기, routine 그래프 재사용, 독립 입력 view와 네 종류 외부 입력 고정 회귀 시나리오가 포함된다. 실제 native 검증 프로세스와 합성 worker를 사용했다. 같은 source hash의 이미지를 실제 kind에서 다시 검증했으며 `kubernetes-grant-validation.json`에 격리 receipt와 namespace 삭제 결과를 기록했다.
...
## 불변 Plan Memory와 이관 계보

`PlanMemory`는 활성 계획의 각 노드를 task 상태 한 행으로 덮어쓰지 않고 append-only `plan_node_versions`로 투영한다. 목표, 기대, 실제 관찰, prediction error, dependency view, 가정, 결정, 경계, 무효화 원인과 근거를 같은 버전에 묶는다. 등록 외부 입력이 unknown·missing·철회 상태여도 투영을 실패시키지 않고 미확정 hash를 계보에 남긴다. 같은 의미 상태의 event 재전달은 새 버전을 만들지 않는다. worker 실행과 검증이 진행되면 같은 plan/node의 역사에서 예상→실제→오차→검증 상태를 재구성할 수 있다.

구버전 DB 이관은 source를 읽기 전용으로 열고 모든 table row를 `legacy_records`에 불변 보존한다. 기본 키가 없는 동일 task의 worker/cancel/archive 이력도 행 순서를 포함한 고유 키로 보존한다. source 전체 hash, table별 행 수, 총 레코드 수와 dependency/contract completeness unknown을 `legacy_migration_audits`에 기록한다. 같은 source의 내용이 이후 바뀌면 이전 이관과 섞지 않고 거절하며, 과거 cognition/policy evidence는 합성하지 않는다.
...
1. 파일 gateway 밖의 등록된 코드·도구·환경·외부 입력 관찰은 실행 벡터와 native/Pod 복구 identity에 연결했다. 등록 watcher의 경계 밖 변경 보고도 소유 task의 보수적 무효화와 재계획으로 연결했다. host의 1초 운영 제어 루프가 활성 요청에 등록된 유한 파일 관찰 예산으로 이전 gateway 입력을 자동 재측정한다. 실제 모델 제공자의 난수 채널은 `unknown`이다.
2. finite 영역 선택과 keep/switch, 독립 region 병렬 계획, 모델·integration·expected failure 및 adapter가 제출한 운영 비용 영수증의 보정은 연결했다. dispatcher 중단 전이는 append-only 원장을 남기며 interruption과 중단 전 discarded work를 자동 측정한다. native 교체 세션 생성 시간과 Pod 교체 worker 준비 시간은 warm session loss로, 검증된 archive 복원·중지 PVC 복사·reuse archive 복원은 data migration으로 각 실행 adapter가 불변 영수증을 생산한다.
3. worker·planner·replanner L5와 plan-node memory는 연결했다. 측정 연구로 승격된 활성 정책은 같은 validator·조건·권한·표본 유형과 고정 rollback을 쓰는 회귀 watch를 자동 등록한다. 모든 13개 정책 target은 요청별 고정 bundle과 target별 typed runtime adapter를 통해 실제 제어 경로에 연결했다. PolicyProposal은 supporting cases, counterexamples, structural abstraction, holdout criteria, rollback condition과 target 일치 effect를 필수로 검증한다. observed paired holdout의 성공 실행은 실제 provider 호출 예외에 포함된다.
4. 구버전 모든 원시 레코드와 task 계보의 무손실 격리 이관, macOS·Linux native 격리와 unsupported 환경의 Pod 강제는 연결했다. host는 grant 이전 worker를 한 번씩 중지하고 실제 abort·idle 확인 뒤에만 예약을 해제하며, 같은 task/spec을 새 activation grant 대기로 복귀시킨다. 이전 attempt와 이관 증거는 append-only로 보존한다. T15의 세 시나리오는 실제 runtime event·증거·정책 proposal을 순서대로 검사하는 불변 `ArchitectureAcceptanceTrace`로 연결했다. 성공한 실제 provider 호출은 사용자가 유지하기로 한 예외다.

`implementation-status.json`의 endToEndVerified는 위 전체 수용 조건과 T15 불변 trace를 기준으로 기록한다. 실제 provider 성공 호출 예외는 각 요구의 상태에 명시한다.
