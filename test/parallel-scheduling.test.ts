...
test("별도 MCP 작업자 프로세스가 동시에 같은 파일을 점유하면 하나만 시작한다", async (t) => {
  ...
  try {
    for (const client of clients) await client.connect(new StdioClientTransport({ command: process.execPath, args: [resolve("scripts/graph-mcp.ts"), path], env: {TASK_GRAPH_SURFACE:"controller"}, stderr: "pipe" }))
