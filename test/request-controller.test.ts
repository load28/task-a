...
test("실제 의미 실패는 필요한 QA만 깨우며 quota와 결과 검증 의무를 유지한다",async()=>{
  ...
  try {
    ...
    new PolicyLearning(r.store.control).propose({id:"architect-trigger",version:1,target:"activation",observedPattern:"실패 관찰",rootCause:"구조 검토 판단",proposedInvariant:"필수 의무 보존",proposedRule:{op:"gte",feature:"failure",value:.5},effect:{kind:"activation",role:{id:"architect",version:1},mode:"require"},expectedBenefit:1,regressionRisk:.1,evidence:[{id:"operator",version:1}],supportingCases:[{id:"operator",version:1}],counterexamples:[],structuralAbstraction:"failure signal",holdoutCriteria:["independent request"],rollbackCondition:"critical regression",rollback:baseline.policy},ref=>r.control.evidence.valid(ref))
