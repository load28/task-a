...
for(const mode of ["regressed","unchanged","malformed","failed","stale","aba","restart","observed-only","execution-error","truncated"] as const)test(`실제 정책 회귀 검증과 head fencing: ${mode}`,async()=>{
  ...
  try {
    ...
    const proposal:PolicyProposal={id:"learned",version:1,target:"activation",observedPattern:"관측 결과",rootCause:"호출 정책",proposedInvariant:"위험 보존",proposedRule:{op:"gte",feature:"risk",value:.5},effect:{kind:"activation",role:{id:"reviewer",version:1},mode:"require"},expectedBenefit:1,regressionRisk:.1,evidence:[authorization],supportingCases:[authorization],counterexamples:[],structuralAbstraction:"risk band",holdoutCriteria:["independent episode"],rollbackCondition:"quality regression",rollback:{id:"learned",version:1}}
