...
test("atomic lifecycle: engine decides transitions from submitted results", (t) => {
  ...
  assert.equal(load.completion.complete, false)
  assert.ok(load.completion.missing.some(reason => reason.includes("unresolved issue")))
  assert.equal(engine.feedback.list(root.id).length, 1)
...
test("recursive decomposition keeps hierarchy and dependencies separate", (t) => {
  ...
  const runnable = engine.resolveRunnable(root.id)
  assert.deepEqual(runnable.map((item) => item.task.title).sort(), ["Pattern IR"], "nested work waits for inherited upstream inputs before pull")
