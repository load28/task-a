...
function setup(r:ReturnType<typeof createGraphRuntime>,command?:string) {
  ...
  const policyProfile={id:"policy-minimum",level:3 as const,provider:"test",model:"bounded",maxInputTokens:1000,maxOutputTokens:100,maxToolCalls:1,timeoutMs:60000,capability:{usage:true,tokenLimit:true,toolLimit:true,timeout:true},independentRoles:[]}
  const proposal:PolicyProposal={id:"precision",version:2,target:"precision",observedPattern:"반복 검증",rootCause:"정밀도",proposedInvariant:"검증 보존",proposedRule:{op:"gte",feature:"risk",value:.5},effect:{kind:"precision",minimumProfile:policyProfile},expectedBenefit:1,regressionRisk:.1,evidence:[authorization],supportingCases:[authorization],counterexamples:[],structuralAbstraction:"risk band",holdoutCriteria:["independent episode"],rollbackCondition:"quality regression",rollback:{id:"precision",version:1}}
