...
try {
  ...
  const role:RoleVersion={id:"smoke",version:1,name:"Adapter smoke",purpose:"Bounded native adapter verification",capabilities:[],prompt:`Return exactly this JSON object and nothing else. Do not use tools or markdown: ${JSON.stringify(expected)}`,activationPolicy:{hardTriggers:["failure"],softSignals:{},threshold:.5,cooldownMs:0,maxInvocationsPerTask:1},requiredContext:[],contextBudget:{maxTokens:33000,maxDependencyDepth:0,maxEvidenceItems:0,maxHistoricalDecisions:0},outputSchema:{const:expected},validators:[],allowedTools:[],lifecycle:"temporary",evidence:[]}
  ...
  const grant=r.control.admission.issue({taskId:task.id,decisionId:decision.id,specHash:snapshot.specHash,inputVector:[{entityId:task.id,port:"inputs",view:"legacy-complete-input",version:1,hash:snapshot.digest}],graphHash:r.control.graph.hash(),role:{id:role.id,version:1},policy:{id:"adapter-test",version:1},context:{id:context.id,version:1},contextHash:context.hash,profile:{id:"native-smoke",level:3,provider,model,maxInputTokens:32000,maxOutputTokens:1000,maxToolCalls:1,timeoutMs:60000,capability:{usage:true,tokenLimit:true,toolLimit:true,timeout:true},independentRoles:[]},readScopes:[],writeScopes:[],allowedTools:[],obligations:[],expiresAt:Date.now()+120000,generation:1},"bounded-native-smoke",33000)
