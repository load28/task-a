...
export class RoutineRegistry {
  ...
  available():Array<{routine:VersionRef;name:string;entryNodes:string[];exitNodes:string[];inputPorts:VersionVector;outputPorts:VersionVector}>{
    return this.runtime.store.db.prepare("SELECT id,version FROM control_heads WHERE collection='routine_versions' ORDER BY id").all().flatMap(row=>{try{const routine=this.current({id:String(row.id),version:Number(row.version)}),ids=new Set(routine.nodes.map(item=>item.node.nodeId)),entries=routine.nodes.filter(item=>!item.node.dependsOnNodeIds.some(id=>ids.has(id))).map(item=>item.node.nodeId),depended=new Set(routine.nodes.flatMap(item=>item.node.dependsOnNodeIds));return [{routine:{id:routine.id,version:routine.version},name:routine.name,entryNodes:entries,exitNodes:routine.nodes.filter(item=>!depended.has(item.node.nodeId)).map(item=>item.node.nodeId),inputPorts:routine.inputPorts,outputPorts:routine.outputPorts}]}catch{return []}})
  }
  expand(items:RoutineProposalItem[],targetGoal:string,requestId:string):{proposal:Array<{node:PlanNode;expectation:RoutineExpectation}>;uses:RoutineUse[]} {
