...
import { currentInputVector,observationInputVector } from "./completion.ts"
...
export class AdversarialReview {
  ...
  ingest():number {
    ...
    return store.consume("adversarial-review/v1","independent-grants-and-joint-verdict/v1",event=>{
      if(event.type==="AgentCompleted") {
        ...
        if(!grant||row?.state!=="completed"||grant.profile.level!==5||output.unresolvedQuestions.length||output.requiresEscalation)return
        ...
        const tuple=grant.executionMode==="task"?observationInputVector(this.runtime.engine,grant.taskId):grant.inputVector
        const observation=grant.executionMode==="task"?store.db.prepare("SELECT payload FROM validation_obligations WHERE entity_id=? AND json_extract(payload,'$.kind')='prediction-state' ORDER BY rowid DESC").all(grant.taskId).map(row=>JSON.parse(String(row.payload))).find(obligation=>digest(obligation.tuple)===digest(tuple)):undefined
        if(grant.executionMode==="task"&&!observation)throw new Error("L5 task execution requires a pinned current observation obligation")
        ...
        for(const id of grant.profile.independentRoles) {
          ...
          const decision=this.runtime.policyReplay.recordActivation({taskId:grant.taskId,eventId:`adversarial:${grant.id}:${id}`,eligible:grant.executionMode==="cognition"||this.runtime.engine.store.executionAllowed(grant.taskId),role,policy:grant.policy,signals,now:event.timestamp,invocations:prior.length,lastInvocation:prior[0]?JSON.parse(String(prior[0].payload)).timestamp:undefined,requiredBy:proof},request.id,[proof,...inputBoundaryEvidence(grant)])
          decisions.push(decision.id)
          store.db.prepare("INSERT INTO specialist_demands VALUES(?,?,1,'deferred',NULL,NULL,?)").run(decision.id,grant.taskId,canonical({requestId:request.id,entry,proofRef:proof,...(observation?{obligationId:observation.id}:{}),signals,budget:"",kind:"adversarial",sourceGrant:grant.id,requiredBy:proof}))
        }
        store.db.prepare("INSERT INTO adversarial_reviews VALUES(?,?,?,'reviewing',NULL,?)").run(grant.id,request.id,grant.taskId,canonical({decisions,proof,validator:program.adversarialValidator,tuple,executionMode:grant.executionMode}))
      }
  ...
  status(sourceGrant:string):"pending"|"satisfied"|"failed" {
    const row=this.runtime.store.db.prepare("SELECT state FROM adversarial_reviews WHERE source_grant=?").get(sourceGrant)
    return row?.state==="satisfied"?"satisfied":row?.state==="failed"?"failed":"pending"
  }
  private current(row:Record<string,unknown>,payload:{tuple:VersionVector;executionMode?:"cognition"|"task"}):boolean {
    try {
      if(payload.executionMode!=="cognition")return digest(payload.tuple)===digest(observationInputVector(this.runtime.engine,String(row.task_id)))
      const grantRow=this.runtime.store.db.prepare("SELECT state,payload FROM activation_grants WHERE id=?").get(String(row.source_grant))
      if(grantRow?.state!=="completed")return false
      const grant=JSON.parse(String(grantRow.payload)) as ActivationGrant
      return digest(payload.tuple)===digest(grant.inputVector)&&digest(grant.inputVector)===digest(currentInputVector(this.runtime.engine,grant.taskId))
    }catch{return false}
  }
  private advance(row:Record<string,unknown>):void {
    const {store,evidence}=this.runtime,payload=JSON.parse(String(row.payload)) as {decisions:string[];proof:VersionRef;validator:string;tuple:VersionVector;executionMode?:"cognition"|"task"}
    if(!this.current(row,payload))return
    if(row.state==="validating") {
      ...
      if(evidence.satisfied(obligation)) {
        ...
        if(payload.executionMode!=="cognition")this.runtime.completeObserved(String(row.task_id))
