  ...
  ingest():number {
    ...
    return store.consume("request-outcomes/v1","accepted-measurements/v1",event=>{
      ...
      const runs=rows.map(row=>{
        ...
        return {grantId:grant.id,taskId:grant.taskId,role:grant.role,policy:grant.policy,profile:grant.profile,state:String(row.state),usage:result?.usage??null,outputHash:result?.output?digest(result.output):null}
      })
