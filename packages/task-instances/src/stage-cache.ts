...
export function stageKey(spec: InstanceSpec, stage: InstanceSpec["stages"][number], completed: Record<string, string>) {
  ...
  return hash([...(spec.activation?[spec.activation]:[]),spec.inputSnapshot?.digest ?? null, spec.image, stage.command, stage.outputs ?? [], stage.inputDigest ?? spec.repository ?? null,
