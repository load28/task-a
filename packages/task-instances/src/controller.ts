...
export function podFor(instance: TaskInstance, sourceVolumes: string[] = [], sourceArchives: Array<WorkspaceArchive | null> = [], restoreSource?: { claim: string; archive?: WorkspaceArchive }): Resource {
  ...
  const result: Resource = { apiVersion: "v1", kind: "Pod", metadata: {
    ...
    ...(instance.spec.activation?{activeDeadlineSeconds:Math.max(1,Math.ceil((instance.spec.activation.expiresAt-Date.now())/1000))}:{}),
    ...
    containers: [{ name: "worker", image: instance.spec.image, imagePullPolicy: "IfNotPresent",
      ...
      env: [{ name: "TASK_INSTANCE_SPEC", value: JSON.stringify(instance.spec) }, { name: "TASK_INSTANCE_ID", value: instance.metadata.uid },
        ...(instance.spec.activation?[{name:"TASK_ACTIVATION_GRANT",value:instance.spec.activation.grantId},{name:"TASK_ACTIVATION_TASK",value:instance.spec.activation.taskId}]:[]),
      ...
      ...((instance.spec.envSecret||instance.spec.activation)?{envFrom:[...(instance.spec.envSecret?[{secretRef:{name:instance.spec.envSecret}}]:[]),...(instance.spec.activation?[{secretRef:{name:instance.spec.activation.authoritySecret}}]:[])]}:{}),
