...
export const instanceTools = [
  { name: "task_instance_create", description: "Create the durable Kubernetes execution environment for an existing runnable leaf. Stages run argv commands in order. Pass exact repository commit and environment image. Never run the same task in a native subagent as well.",
...
export const INSTANCE_INSTRUCTIONS = `Kubernetes execution is enabled. For runnable leaf work use task_instance_create instead of native task subagents or local execution. Creation atomically claims the graph leaf for Kubernetes; do not call task_start again for that leaf. Each instance owns an isolated checkout and durable OpenCode session data. Load task_instance_status when resuming from another host conversation. Stop and wait for Suspended before resuming with the next run number. A completed Pod is only command completion, not graph verification: publish evidence and complete the graph task separately. Pass required tools, remote graph connection and credentials through a preconfigured environment image/Secret; never put credentials in instance specifications. Follow the leaf dependencies and worker capacity. Transfer verified commits to the integration instance before downstream validation. Do not assume isolated files have been merged into the user's checkout.`
