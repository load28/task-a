...
export class OpenCodeReasoner implements TaskReasoner {
  ...
  private async prompt(title: string, text: string, schema?: Record<string, unknown>, readOnly = true) {
    ...
    const result = await this.client.session.prompt({
      ...
      tools: Object.fromEntries(taskTools.map((name) => [name, !readOnly])),
    ...
    if (result.data.info.error) throw new Error(`OpenCode failed: ${result.data.info.error.name}`)
    return result.data
  }
  ...
  async extractEvents(input: { conversation: string; instruction?: string; task: TaskRecord }): Promise<ExtractedEvent[]> {
    const result = await this.prompt("Task sync",
      `Extract durable events only; do not call mutation tools. Return [] for proposals, questions and duplicate facts. For artifacts include metadata.uri and metadata.type; for status include metadata.status; resolve blockers by metadata.resolves and replace decisions by metadata.supersedes using existing event IDs.\n\n${JSON.stringify({ instruction: input.instruction ?? "Extract task-worthy events.", task: serializeTask(input.task), conversation: input.conversation })}`,
    ...
    return output.events as ExtractedEvent[]
  }
  ...
  properties: {
    events: {
      ...
      items: {
        type: "object",
        properties: {
          type: { type: "string", enum: ["decision", "progress", "finding", "constraint", "blocker", "blocker_resolved", "next_action", "artifact", "status"] },
          content: { type: "string" },
...
function serializeTask(record: TaskRecord) {
  ...
  const active = record.events.filter((event) => ["decision", "blocker"].includes(event.type) && !replaced.has(event.id))
