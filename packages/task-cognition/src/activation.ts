...
export function activation(input:{taskId:string;eventId:string;eligible:boolean;role:RoleVersion;policy:VersionRef;signals:Signals;now:number;lastInvocation?:number;invocations:number}):ActivationDecision {
  ...
  if(!input.eligible) reasons.push("outside causal eligibility")
    ...
    const wanted=hard.length>0||score>policy.threshold
    ...
    if(wanted||unknown||unknownHard) {
      ...
      else {action="activate";reasons.push(hard.length?"hard evidence trigger":"soft score exceeded threshold")}
